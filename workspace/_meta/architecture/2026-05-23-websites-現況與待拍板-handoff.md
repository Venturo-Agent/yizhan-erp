...
**還沒做（spec 的核心）：**
- ❌ DB：workspaces 加 `subdomain` / `canvas` jsonb / canvas_updated/published_* 審計欄位
...
| 3 | public 站 SEO？ | v1：workspace 級 SEO defaults + tour 級 override（用 tours 既有 seo_* 欄位）|
...
*記錄人：主 Claude。觸發：William 提醒「討論要寫進檔案、不然新對話會丟」（本對話開場原則）。*
