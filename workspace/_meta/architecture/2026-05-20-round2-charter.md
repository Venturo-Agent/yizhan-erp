...
> 「3 個 cis\_\* table（cis_clients / cis_pricing_items / cis_visits）完全不存在、但前端 3 個 page (`/cis/page.tsx` / `/cis/[id]/page.tsx` / `/cis/pricing/page.tsx`) + 3 個 entity hook 存在、用戶進去 hook call 會炸」

這是 6 層 audit **該抓但漏的 L1 + L5 對齊問題**。

**Round 2 必做**：

1. 證實 3 個 cis\_\* page 存在
2. 證實 3 個 cis\_\* entity hook 存在
3. 證實 3 個 cis\_\* DB table **不存在**（grep migrations 是否曾 CREATE）
