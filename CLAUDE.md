...
### 6. Supabase 用 MCP 接、部署走 GitHub → Vercel

- 連 yizhan-erp Supabase 用 **`mcp__supabase-aierp__*`** 全家（`execute_sql` / `apply_migration` / `list_tables` / `generate_typescript_types` 等、MCP 名稱保留 `aierp`、不影響功能）— project_id 是 `aawrgygqgemgqssflfrx`
- 不再走 SSH 中介、不用本機 psql（Mac IPv6 不通也沒關係、MCP 直接打 Supabase）
- 部署：**`git push` → GitHub → Vercel 自動部署**、沒有額外步驟

### 7. 報告問題用白話 + 譬喻、技術術語放括號裡

- 講「會發生什麼後果」（全 app 黑屏 / 員工進不去 / 客戶資料看不到）、不只「哪行 code 出錯」
- 譬喻用日常物件（守門員 / 識別證 / 飯店 / 工作手冊）、不用 RLS / function / transaction 當主體
- 救護車式排序：先講「會死人嗎」、再講「怎麼治」、最後才講「醫學原理」
- William 不看 code、要用業務人聽得懂的話描述技術問題
...
```
→ GitHub 觸發 Vercel webhook → 自動部署。

- `$VERCEL_TOKEN` / `$VERCEL_PROJECT_ID` 已配、不用手動 deploy
- **❌ 不准**：SSH 任何 server、Coolify、手動 Docker、`vercel` CLI（除非明說）
...
- **🗄️ 已棄用**（2026-05-18 改 Vercel 後）：`VULTR_*`、`COOLIFY_*`、`HETZNER_*`、`VENTURO_AIERP_COOLIFY_APP_UUID`
...
8. `docs/db-state-snapshot-2026-05-13.md` — production DB 真實狀態（Markdown 給人讀）
9. `docs/api-capability-audit-2026-05-13.md` — 76 API route 守門 audit 結果（0 缺）

看完這 9 份就懂全貌、不用花一個 session 重新摸索。
...
6. **Push**：apply 成功 + 驗證過後 `git push` → **GitHub → Vercel 自動部署**
