...
const Textarea = React.forwardRef<HTMLTextAreaElement, React.ComponentProps<'textarea'>>(
  ({ className, onChange, onKeyDown, onCompositionStart, onCompositionEnd, ...props }, ref) => {
    ...
    return (
      <textarea
        className={cn(
          'flex min-h-[5rem] w-full rounded-md border border-input bg-card px-3 py-2 text-base font-medium placeholder:text-muted-foreground placeholder:font-normal focus:outline-none focus:ring-0 focus:border-morandi-gold disabled:cursor-not-allowed disabled:bg-input-disabled-bg disabled:text-morandi-primary/85 read-only:bg-input-readonly-bg md:text-sm transition-colors',
          className
        )}
        ref={ref}
