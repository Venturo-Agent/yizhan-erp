...
export async function GET(
  ...
  const { data: batch, error: batchErr } = await s
    ...
    .select('id, workspace_id, order_id, public_token, token_expires_at, status, notes, created_at')
  ...
  if (new Date(batch.token_expires_at) < new Date()) {
    return NextResponse.json(
      { error: '此連結已過期、請聯絡業務員重發' },
      { status: 410 }
    )
  }

  // 平行撈：invoices / order / workspace / receipts / payment_methods
  const [invoicesResult, orderResult, workspaceResult, receiptsResult, paymentMethodsResult] =
    await Promise.all([
      s
        .from('invoices')
        .select(
          `id, customer_id, member_id, total_amount, paid_amount, status, notes, created_at`
        )
  ...
  const [customersResult, membersResult] = await Promise.all([
    customerIds.length
      ? s
          .from('customers')
          .select('id, chinese_name, passport_name')
          .in('id', customerIds)
  ...
  const customerMap = new Map(
    (customersResult.data || []).map((c: { id: string; chinese_name: string | null; passport_name: string | null }) => [
      c.id,
      c.chinese_name || c.passport_name || '客戶',
    ])
  )
  const memberMap = new Map(
    (membersResult.data || []).map((m: { id: string; chinese_name: string | null; passport_name: string | null }) => [
      m.id,
      ...
      ? {
          ...
          bank: workspace.bank_account
            ? {
                ...
                bank_code: workspace.bank_code,
                branch: workspace.bank_branch,
                account: workspace.bank_account,
                account_name: workspace.bank_account_name,
              }
            : null,
        }
      : null,
