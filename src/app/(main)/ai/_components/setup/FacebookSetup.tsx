...
/**
 * /bot/facebook-setup — Facebook Messenger 自助開通 Setup Wizard
 *
 * 5 step：
 *   1. Welcome 教學
 *   2. Credentials 填 page_access_token（+ 可選 app_secret + greeting）
 *   3. Validate（call /api/facebook/setup/validate-credentials、回 pageId / pageName）
 *   4. Provisioning（call /api/facebook/setup/provision、加密寫 DB、回 webhook URL + verify token）
 *   5. WebhookUrl 顯示 webhook URL + verify token 教客戶貼回 Meta App
 *
 * 視覺骨架走共用組件 SetupWizardLayout / StepIndicator（跟 LINE setup 一致）。
 * 步驟 UI 子組件在 FacebookSetupSteps.tsx。
 *
 * 5/14 移植自 src/app/(main)/bot/facebook-setup/page.tsx、變 component 用在 /ai?tab=setup
...
} from './FacebookSetupSteps'

type Step = 'welcome' | 'credentials' | 'validate' | 'provision' | 'done'

const FB_SETUP_STEPS: ReadonlyArray<SetupStep<Step>> = [
...
export function FacebookSetup() {
  ...
  const handleValidate = async () => {
    ...
    setValidating(true)
    try {
      const res = await fetch('/api/facebook/setup/validate-credentials', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page_access_token: credentials.page_access_token }),
      })
      ...
      setPageInfo(json.data as PageInfo)
      setStep('validate')
    } catch (e) {
      logger.error('FB validate failed', e)
  ...
  const handleProvision = async () => {
    ...
    setStep('provision')
    try {
      const res = await fetch('/api/facebook/setup/provision', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      setStep('done')
      toast.success('FB Messenger 開通成功')
    } catch (e) {
      logger.error('FB provision failed', e)
      setError(e instanceof Error ? e.message : '無法連 API')
      setStep('validate')
    } finally {
  ...
  return (
    <SetupWizardLayout title="Facebook Messenger 整合" icon={Facebook}>
      <StepIndicator steps={FB_SETUP_STEPS} current={step} />
      ...
      <Card className="p-6">
        ...
        {step === 'done' && provisionResult && (
          <StepDone
            ...
            pageName={provisionResult.pageName}
            onCopy={handleCopy}
          />
        )}
      </Card>
