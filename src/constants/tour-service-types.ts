...
export const TOUR_SERVICE_TYPES: readonly TourServiceTypeMeta[] = [
  ...
  },
  {
    id: 'esim',
    label: '網卡',
    description: 'eSIM 訂單管理（不選國家機場、prefix ESIM）',
    codePrefix: 'ESIM',
  ...
  },
  {
    id: 'visa',
    label: '簽證',
    description: '簽證代辦（不選國家機場、prefix VISA）',
    codePrefix: 'VISA',
    needsDestination: false,
    needsItinerary: false,
    needsController: false,
  ...
  },
  {
    id: 'outsource',
    label: '外丟團',
    description: '外包給其他旅行社（不選國家機場、prefix OUT）',
    codePrefix: 'OUT',
    needsDestination: false,
    needsItinerary: false,
    needsController: false,
...
export const TOUR_TYPE_CODE_PREFIX: Record<string, string> = Object.fromEntries(
  TOUR_SERVICE_TYPES.filter(t => !t.needsDestination && t.codePrefix).map(t => [
    t.id,
    t.codePrefix!,
  ])
)
