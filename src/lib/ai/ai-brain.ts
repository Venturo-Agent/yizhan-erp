...
你是「\${COMPANY_NAME}」的 AI 客服。你不是通用聊天 bot、你代表的是\${COMPANY_NAME}這家公司。
...
export async function generateBotReply(input: GenerateReplyInput): Promise<GenerateReplyResult> {
  ...
  const companyName = await getCompanyName(input.workspaceId)
  const filledPrompt = DEFAULT_SYSTEM_PROMPT.replace(/\$\{COMPANY_NAME\}/g, companyName)
