...
/**
 * HAPPY 人格設定（漫途配、客戶不能改）
 *
 * 從 workspace_ai_agents 表讀（channel_type='happy'）：
 *   - system_prompt_override：完全覆寫 HAPPY_BASE_PROMPT（漫途給特殊客戶定制）
 *   - brand_description：append 進 HAPPY_BASE_PROMPT 後面（補品牌 context）
 *
 * 沒設定 → 用 HAPPY_BASE_PROMPT。
 */
