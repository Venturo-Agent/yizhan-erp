-- ─────────────────────────────────────────────────────────────────────────────
-- 新租戶預設只建一個「主要帳戶」（取代 現金 / 主要銀行帳戶 / 備用銀行帳戶 三個）
--
-- 寫於：2026-05-29
-- Why（William）：開新公司時預設塞 3 個銀行帳戶（現金/主要/備用）多餘、代號也無意義。
--   統一只給一個「主要帳戶」、設為預設、其餘讓客戶自己加。
--
-- 只改 create_default_finance_settings() 的「3. 銀行帳戶」段、其餘（收付款方式 / 會計科目）原樣保留。
-- 既有租戶不動（只影響之後新建的租戶）。
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ════ Rollback（回到 3 個銀行帳戶）：把 §3 改回 CASH/BANK1/BANK2 三筆、重跑 CREATE OR REPLACE ════
