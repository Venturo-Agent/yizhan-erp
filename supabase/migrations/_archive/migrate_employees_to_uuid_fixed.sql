-- ============================================
-- 架構重構：將 employees.id 遷移到 UUID（修正版）
-- ============================================
-- 修正：加入類型轉換，處理 UUID 和 TEXT 的比較
-- ============================================
...
BEGIN
  IF EXISTS (
    ...
    WHERE o.employee_id::text = m.old_id;  -- ✅ 類型轉換
...
BEGIN
  IF EXISTS (
    ...
    FROM employees e
    WHERE m.author_id::text = e.id;  -- ✅ 將 UUID 轉為 TEXT 比較
...
BEGIN
  IF EXISTS (
    ...
    SET author_id_new = e.uuid_new
    FROM employees e
    WHERE b.author_id::text = e.id;  -- ✅ 類型轉換
...
BEGIN
  IF EXISTS (
    ...
    FROM employees e
    WHERE c.created_by::text = e.id;  -- ✅ 類型轉換
...
BEGIN
  IF EXISTS (
    ...
    SET created_by_new = e.uuid_new
    FROM employees e
    WHERE w.created_by::text = e.id;  -- ✅ 類型轉換
...
BEGIN
  IF EXISTS (
    ...
    SET created_by_new = e.uuid_new
    FROM employees e
    WHERE al.created_by::text = e.id;  -- ✅ 類型轉換
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE advance_items ADD COLUMN IF NOT EXISTS processed_by_new UUID;

    -- ✅ 修正：加入類型轉換
    UPDATE advance_items ai
    SET processed_by_new = e.uuid_new
    FROM employees e
    WHERE ai.processed_by::text = e.id;  -- ✅ 類型轉換

    RAISE NOTICE '✅ advance_items.processed_by 已更新';
  END IF;
END $$;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'employee_id_new') THEN
    ALTER TABLE orders DROP COLUMN IF EXISTS employee_id;
    ALTER TABLE orders RENAME COLUMN employee_id_new TO employee_id;
    ALTER TABLE orders ADD FOREIGN KEY (employee_id) REFERENCES employees(id) ON DELETE SET NULL;
  END IF;

  -- Messages
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'messages' AND column_name = 'author_id_new') THEN
    ALTER TABLE messages DROP COLUMN IF EXISTS author_id;
    ALTER TABLE messages RENAME COLUMN author_id_new TO author_id;
    ALTER TABLE messages ADD FOREIGN KEY (author_id) REFERENCES employees(id) ON DELETE CASCADE;
  END IF;

  -- Bulletins
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bulletins' AND column_name = 'author_id_new') THEN
    ALTER TABLE bulletins DROP COLUMN IF EXISTS author_id;
    ALTER TABLE bulletins RENAME COLUMN author_id_new TO author_id;
    ALTER TABLE bulletins ADD FOREIGN KEY (author_id) REFERENCES employees(id) ON DELETE CASCADE;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'channels' AND column_name = 'created_by_new') THEN
    ...
    ALTER TABLE channels ADD FOREIGN KEY (created_by) REFERENCES employees(id) ON DELETE SET NULL;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'workspaces' AND column_name = 'created_by_new') THEN
    ...
    ALTER TABLE workspaces ADD FOREIGN KEY (created_by) REFERENCES employees(id) ON DELETE SET NULL;
  END IF;

  -- Advance Lists
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'advance_lists' AND column_name = 'created_by_new') THEN
    ALTER TABLE advance_lists DROP COLUMN IF EXISTS created_by;
    ALTER TABLE advance_lists RENAME COLUMN created_by_new TO created_by;
    ALTER TABLE advance_lists ADD FOREIGN KEY (created_by) REFERENCES employees(id) ON DELETE CASCADE;
  END IF;

  -- Advance Items
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'advance_items' AND column_name = 'processed_by_new') THEN
    ALTER TABLE advance_items DROP COLUMN IF EXISTS processed_by;
    ALTER TABLE advance_items RENAME COLUMN processed_by_new TO processed_by;
    ALTER TABLE advance_items ADD FOREIGN KEY (processed_by) REFERENCES employees(id) ON DELETE SET NULL;
  END IF;
END $$;
