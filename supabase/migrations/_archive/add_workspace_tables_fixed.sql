-- ============================================
-- Workspace 工作空間系統資料表（修正版）
-- ============================================
-- 建立日期：2025-01-22
-- 說明：支援工作空間、頻道、訊息、代墊清單等功能
-- 修正：處理已存在的表，使用 ALTER TABLE 添加欄位
-- ============================================
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'workspaces' AND column_name = '_needs_sync') THEN
    ALTER TABLE workspaces ADD COLUMN _needs_sync BOOLEAN DEFAULT false;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'workspaces' AND column_name = '_synced_at') THEN
    ALTER TABLE workspaces ADD COLUMN _synced_at TIMESTAMP WITH TIME ZONE;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name = 'workspaces' AND column_name = '_deleted') THEN
