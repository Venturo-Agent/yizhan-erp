-- ============================================
-- Workspace 工作空間系統資料表（最終版）
-- ============================================
-- 建立日期：2025-01-22
-- 說明：支援工作空間、頻道、訊息、代墊清單等功能
-- 修正：移除對 users 表的外鍵約束（因為 users 表不存在）
-- ============================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'workspaces' AND column_name = 'created_by') THEN
    ALTER TABLE workspaces ADD COLUMN created_by UUID;  -- 不設外鍵，因為 users 表不存在
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'channels' AND column_name = 'type') THEN
    ALTER TABLE channels ADD COLUMN type VARCHAR(20) DEFAULT 'public';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'channels' AND column_name = 'tour_id') THEN
    ALTER TABLE channels ADD COLUMN tour_id TEXT;  -- tours.id 是 text 類型，且不設外鍵避免依賴問題
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'channels' AND column_name = 'created_by') THEN
    ALTER TABLE channels ADD COLUMN created_by UUID;  -- 不設外鍵
  END IF;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bulletins' AND column_name = 'type') THEN
    ALTER TABLE bulletins ADD COLUMN type VARCHAR(20) DEFAULT 'notice';
  END IF;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'bulletins_type_check'
  ) THEN
    ALTER TABLE bulletins ADD CONSTRAINT bulletins_type_check
      CHECK (type IN ('announcement', 'notice', 'event'));
  END IF;
END $$;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'advance_items' AND column_name = 'status') THEN
    ALTER TABLE advance_items ADD COLUMN status VARCHAR(20) DEFAULT 'pending';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'advance_items' AND column_name = 'payment_request_id') THEN
    ALTER TABLE advance_items ADD COLUMN payment_request_id UUID;  -- 不設外鍵，避免依賴問題
  END IF;

  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'advance_items' AND column_name = 'processed_by') THEN
    ALTER TABLE advance_items ADD COLUMN processed_by UUID;  -- 不設外鍵
  END IF;
...
-- 添加 status 欄位的 CHECK 約束（如果不存在）
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'advance_items_status_check'
  ) THEN
    ALTER TABLE advance_items ADD CONSTRAINT advance_items_status_check
...
-- Workspaces 政策：所有人可讀寫（因為沒有 auth.uid()）
